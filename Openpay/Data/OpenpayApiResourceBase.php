...
abstract class OpenpayApiResourceBase
{
    ...
    protected function __construct($resourceName, $params = [])
    {
        $this->resourceName = $resourceName;
        $this->serializableData = [];
        $this->noSerializableData = [];
    ...
    private function getResourceName($name)
    {
        ...
        if ('Openpay' == substr($name, 0, \strlen('Openpay'))) {
            return $name;
        }
    ...
    private function registerInParent($resource): void
    {
    ...
    protected function getResourceUrlName($pluralize = true)
    {
        ...
        $class = $ResourceUrl[\count($ResourceUrl) - 1];
        if ('Openpay' == substr($class, 0, \strlen('Openpay'))) {
            $class = substr($class, \strlen('Openpay'));
        }
        if ('List' == substr($class, -1 * \strlen('List'))) {
            $class = substr($class, 0, -1 * \strlen('List'));
        }
        if ('Pse' == substr($class, -1 * \strlen('OpenpayPse'))) {
            $class = 'Pse';
    ...
    protected function validateId($id): void
    {
        ...
        if ('Bine' != substr($class, -1 * \strlen('Bine'))) {
    ...
    public function __get($key)
    {
        if (property_exists($this, $key)) {
            return $this->$key;
        } elseif (\array_key_exists($key, $this->serializableData)) {
            return $this->serializableData[$key];
        } elseif (\array_key_exists($key, $this->derivedResources)) {
            return $this->derivedResources[$key];
        } elseif (\array_key_exists($key, $this->noSerializableData)) {
